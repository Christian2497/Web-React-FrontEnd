...
//definimos 3 métodos (relacionados con auth-service.js):
  ...
  render() {
    ...
    const { login, logout, signup, addExercise, allVideos, userInfo, exerciseInfo } = this;

    return isLoading ? (
      <div>Loading</div>
    ) : (
      <Provider value={{ isLoggedin, user, login, logout, signup, allVideos, addExercise, userInfo, exerciseInfo }}>
        {this.props.children}
      </Provider>
    );
  }
